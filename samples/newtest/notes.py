...
FULLSET = np.array([fn for fn in os.listdir(ROOT_DIR + '/datasets/midis/mask/') if fn.endswith(".wav.png")])
...
class NotesDataset(utils.Dataset):
    ...
    def load_notes(self, settype=None):
        ...
        if settype == "train":
            ...
            fnset = VALSET
        for fn in fnset:
            if os.path.isfile(ROOT_DIR + "/datasets/midis/mask/" + fn[:-8] + '_mask.npz'):
                self.add_image("notes", image_id=fn[:-8], path=ROOT_DIR + "/datasets/midis/mask/" + fn)
    ...
    def load_mask(self, image_id):
        ...
        """
        info = self.image_info[image_id]
        mask = np.load(info['path'][:-8] + '_mask.npz')['mask']
        notes = np.load(info['path'][:-8] + '_notes.npy')
